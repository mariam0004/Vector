    ...
    , helpAction(new QAction(tr("Help"), parent))
{
    ...
    QMainWindow* mainWindow = qobject_cast<QMainWindow*>(parent);//dyn cast to ptr to QMainWndow
    if (mainWindow) {//if succes dyn cast so
        mainWindow->setMenuBar(menuBar);
        qDebug() << "Menu bar set successfully!";//for debug output
    } else {
        qDebug() << "Parent is not a QMainWindow!";
    }


    // Set up the buttons' geometries
...
void LinkedListVisualizer::visualizeLinkedList() {
    ...
    int x = (width() - nodeWidth) / 2;//Divides the available space by 2 to center the linked list horizontally within the QGraphicsView
    Node* current = head;
...
void LinkedListVisualizer::pushFront() {
    ...
    if (!valueStr.isEmpty()) {
        ...
        head -> next = newNode;
        newNode -> color = pushFrontColor; //QColor member pushFrontCOlors, set in ChangeNodeColor function
        visualizeLinkedList();
    }
}
...
void LinkedListVisualizer::pushBack() {
    ...
    if (!valueStr.isEmpty()) {
        ...
        newNode -> color = pushBackColor;//QColor member pushFrontCOlors, set in ChangeNodeColor function
        visualizeLinkedList();
    }
}
...
void LinkedListVisualizer::insertNode() {
    ...
    int position = QInputDialog::getInt(this, "Insert Node", "Enter position", 0, 1, 100, 1, &ok);//getInt function returns the user's input,,+1 for 0-based index
    ...
    if (ok) {
        ...
        if (!valueStr.isEmpty()) {
            ...
            newNode->color = insertColor;//QColor member pushFrontCOlors, set in ChangeNodeColor function
            visualizeLinkedList();
        }
    }
}
